...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
#if (MCU_CORE_TYPE == MCU_CORE_9518)
// *((u32 *)blt_tx_empty_packet) = rf_tx_packet_dma_len(2) = 0x00 80 00 01
//01 00 80 00 01 00
...
int blt_ll_channel_conn_update (u8 *pkt)
{
	...
	if (blc_cyrpt_para.enable)
	{
		...
		blc_cyrpt_para.dec_pno = bak;							//restore original value
	...
	if (p[5])
	{
		...
		if (type == LLID_CONTROL)					//LL control
		{
			...
			else if (opcode == LL_PING_RSP )	//LL Control Opcode: 0x13
			{
				...
				#if (MCU_CORE_TYPE == MCU_CORE_9518)
					rf_set_rx_maxlen(bltData.connEffectiveMaxRxOctets + 4); // Contain MIC(4) length
				#endif
					blt_p_event_callback (BLT_EV_FLAG_DATA_LENGTH_EXCHANGE, (u8 *)&bltData.connEffectiveMaxRxOctets, 12);
			}
#endif
...
void bls_ll_setNewConnection_timing(u16 interval, u8 winsize, u16 winOffset)
{
	...
	if(winOffset == 0 && diff == 1){
		bltc.conn_tolerance_time = 250 * SYSTEM_TIMER_TICK_1US;
		bltc.conn_duration =  500 * SYSTEM_TIMER_TICK_1US + winsize * SYSTEM_TIMER_TICK_1250US;
	}
}
...
u8 blt_brx_start ()
{
...
#elif (MCU_CORE_TYPE == MCU_CORE_9518)
	u8	tx_settle_time;
	//u8	ll_slave_tx_settle_time;
	extern sys_clk_t sys_clk;
	if( (sys_clk.cclk == 64) || (sys_clk.cclk == 96) )
	{
		tx_settle_time = tx_settle_slave_high_freq[blt_conn_phy.conn_cur_phy];
		//ll_slave_tx_settle_time = LL_SLAVE_TX_SETTLE_HIGH_FREQ;
	}
	else
	{
		tx_settle_time = tx_settle_slave[blt_conn_phy.conn_cur_phy];
		//ll_slave_tx_settle_time = LL_SLAVE_TX_SETTLE;
	}

	#if(LL_FEATURE_ENABLE_LE_2M_PHY || LL_FEATURE_ENABLE_LE_CODED_PHY)
		rf_tx_settle_adjust(tx_settle_time);
		reg_rf_rx_timeout = blt_conn_phy.conn_cur_phy == BLE_PHY_CODED ? 600 : 250;
	#else
		rf_tx_settle_adjust(ll_slave_tx_settle_time);
	...
	rf_start_fsm(FSM_BRX,(void *)blt_txfifo.p_default, clock_time () + 10);
#elif (MCU_CORE_TYPE == MCU_CORE_825x || MCU_CORE_TYPE == MCU_CORE_827x)
	rf_start_fsm(FSM_BRX,(void *)blt_tx_empty_packet, clock_time () + 10);
#endif
...
_attribute_ram_code_ void irq_slave_system_timer(void)
{
	...
	void blt_debugTerminateCount(void)
	{
		...
		u32 addr  = 0xF8000;
#endif
		flash_read_page(addr,4,(u8*)&ter_cnt);
		...
		flash_write_page(addr, 4, (u8*)&ter_cnt);
