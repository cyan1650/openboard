...
typedef struct{
	...
	u8						own_conn_addr[6];
	u8						devc_idx;    //local device index
	u8						rsvd;
	u8		                local_irk[16];
}smp_param_own_t;
...
typedef struct {
	...
	u8 cur_bondNum;
		...
		u8	bond_flag[SMP_BONDING_DEVICE_MAX_NUM];
			...
			u32 mldBond_flash_idx[LOCAL_DEVICE_NUM_MAX][SMP_BONDING_DEVICE_MAX_NUM];  //mld: multiple local device
			u8	mldBond_flag[LOCAL_DEVICE_NUM_MAX][SMP_BONDING_DEVICE_MAX_NUM]; //mld: multiple local device
		#endif
	#else
	...
	#if (MULTIPLE_LOCAL_DEVICE_ENABLE)
		u8 mldCur_bondNum[LOCAL_DEVICE_NUM_MAX]; //mld: multiple local device
	#endif
