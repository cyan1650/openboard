...
typedef unsigned char uECC_word_t;
typedef unsigned short uECC_dword_t;
typedef unsigned char wordcount_t;
...
typedef unsigned long long uECC_word_t;
#if SUPPORTS_INT128
...
static int default_RNG(unsigned char *dest, unsigned size) {
    HCRYPTPROV prov;
...
_attribute_data_retention_ static uECC_RNG_Function g_rng_function = &default_RNG;
...
void swECC_set_rng(uECC_RNG_Function rng_function) {
    g_rng_function = rng_function;
        ...
        #include <thirdparty/uecc/asm_avr.inc>
    #endif

    #if (uECC_ASM && (uECC_PLATFORM == uECC_arm || uECC_PLATFORM == uECC_arm_thumb || \
                      uECC_PLATFORM == uECC_arm_thumb2))
        #include <thirdparty/uecc/asm_arm.inc>
    #endif
#endif
...
_attribute_ram_code_ static void muladd(uECC_word_t a,
    ...
    unsigned long long i0 = mul64_32x32(a0, b0);//a0 * b0;
    unsigned long long i1 = mul64_32x32(a0, b1);//a0 * b1;
    unsigned long long i2 = mul64_32x32(a1, b0);//a1 * b0;
    unsigned long long i3 = mul64_32x32(a1, b1);//a1 * b1;
    ...
    *r2 += ((*r1 < p1) || (*r1 == p1 && *r0 < p0));
		...
		uECC_dword_t p = 0;
		uECC_word_t *p0=&p, *p1=(p0+1);
		gmp_umul_ppmm((*p1), (*p0), a, b);

		uECC_dword_t r01;							//  = ((uECC_dword_t)(*r1) << uECC_WORD_BITS) | *r0;
		uECC_word_t *pr10=&r01, *pr11=(pr10+1);
		*pr10 = *r0; *pr11 = *r1;
		...
		*r0 = *pr10;
	#else
		uECC_dword_t p = mul64_32x32(a,b); // a * b;
		uECC_dword_t r01 = ((uECC_dword_t)(*r1) << uECC_WORD_BITS) | *r0;
		r01 += p;
		*r2 += (r01 < p);
...
static void mul2add(uECC_word_t a,
    ...
    unsigned long long i0 = a0 * b0;
    unsigned long long i1 = a0 * b1;
    unsigned long long i2 = a1 * b0;
    unsigned long long i3 = a1 * b1;
...
#if uECC_WORD_SIZE == 1
static void omega_mult(unsigned char * RESTRICT result, const unsigned char * RESTRICT right) {
    unsigned char carry;
    unsigned char i;
    ...
    for (i = uECC_WORDS; carry; ++i) {
        unsigned short sum = (unsigned short)result[i] + carry;
        result[i] = (unsigned char)sum;
        carry = sum >> 8;
    }
}
#elif uECC_WORD_SIZE == 4
static void omega_mult(unsigned int * RESTRICT result, const unsigned int * RESTRICT right) {
    unsigned int carry;
    unsigned i;
    ...
    carry = vli_add(result, result, right); /* 2^31 + 1 */
    for (i = uECC_WORDS; carry; ++i) {
        unsigned long long sum = (unsigned long long)result[i] + carry;
...
#if uECC_WORD_SIZE == 1
static void vli_mmod_fast(unsigned char *RESTRICT result, unsigned char *RESTRICT product) {
    unsigned char tmp[uECC_WORDS];
    unsigned char carry;
...
#elif uECC_WORD_SIZE == 4
static void vli_mmod_fast(unsigned int *RESTRICT result, unsigned int *RESTRICT product) {
    unsigned int tmp[uECC_WORDS];
    int carry;
...
#else
static void vli_mmod_fast(unsigned long long *RESTRICT result, unsigned long long *RESTRICT product) {
    unsigned long long tmp[uECC_WORDS];
    int carry;
...
static void omega_mult(unsigned int * RESTRICT result, const unsigned int * RESTRICT right) {
    ...
    for (k = 0; k < uECC_WORDS; ++k) {
        unsigned long long p = (unsigned long long)0x3D1 * right[k] + carry;
...
#else
static void omega_mult(unsigned long long * RESTRICT result, const unsigned long long * RESTRICT right) {
    uECC_word_t r0 = 0;
    uECC_word_t r1 = 0;
    uECC_word_t r2 = 0;
    wordcount_t k;
...
void vli_mmod_fast(unsigned char *RESTRICT result, unsigned char *RESTRICT product) {
    unsigned char tmp[uECC_WORDS];
    int8_t carry;
...
#elif uECC_WORD_SIZE == 4
void vli_mmod_fast(unsigned int *RESTRICT result, unsigned int *RESTRICT product)
{
    unsigned int tmp[uECC_WORDS];
    int carry;
...
static void vli_nativeToBytes(unsigned char * RESTRICT dest, const unsigned char * RESTRICT src) {
    unsigned char i;
...
static void vli_nativeToBytes(unsigned char *bytes, const unsigned int *native) {
    unsigned i;
    for (i = 0; i < uECC_WORDS; ++i) {
        unsigned char *digit = bytes + 4 * (uECC_WORDS - 1 - i);
        digit[0] = native[i] >> 24;
        digit[1] = native[i] >> 16;
        digit[2] = native[i] >> 8;
        digit[3] = native[i];
...
static void vli_bytesToNative(unsigned int *native, const unsigned char *bytes) {
    unsigned i;
    for (i = 0; i < uECC_WORDS; ++i) {
        const unsigned char *digit = bytes + 4 * (uECC_WORDS - 1 - i);
        native[i] = ((unsigned int)digit[0] << 24) | ((unsigned int)digit[1] << 16) |
                    ((unsigned int)digit[2] << 8) | (unsigned int)digit[3];
    }
}

#else

static void vli_nativeToBytes(unsigned char *bytes, const unsigned long long *native) {
    unsigned i;
    for (i = 0; i < uECC_WORDS; ++i) {
        unsigned char *digit = bytes + 8 * (uECC_WORDS - 1 - i);
        digit[0] = native[i] >> 56;
        digit[1] = native[i] >> 48;
        digit[2] = native[i] >> 40;
        digit[3] = native[i] >> 32;
...
static void vli_bytesToNative(unsigned long long *native, const unsigned char *bytes) {
    unsigned i;
    for (i = 0; i < uECC_WORDS; ++i) {
        const unsigned char *digit = bytes + 8 * (uECC_WORDS - 1 - i);
        native[i] = ((unsigned long long)digit[0] << 56) | ((unsigned long long)digit[1] << 48) |
                    ((unsigned long long)digit[2] << 40) | ((unsigned long long)digit[3] << 32) |
                    ((unsigned long long)digit[4] << 24) | ((unsigned long long)digit[5] << 16) |
                    ((unsigned long long)digit[6] << 8) | (unsigned long long)digit[7];
    }
}
...
int swECC_make_key(unsigned char public_key[uECC_BYTES*2], unsigned char private_key[uECC_BYTES]) {
    ...
    for (tries = 0; tries < MAX_TRIES; ++tries) {
        if (g_rng_function((unsigned char *)private, sizeof(private)) &&
...
int swECC_shared_secret(const unsigned char public_key[uECC_BYTES*2],
    ...
    if (!uECC_valid_public_key(public_key)) {
        return 0;
    }
    ...
    // swECC_shared_secret() can still work without an RNG defined.
    for (tries = 0; tries < MAX_TRIES; ++tries) {
        if (g_rng_function((unsigned char *)random, sizeof(random)) && !vli_isZero(random)) {
            initial_Z = random;
            break;
        }
    }
...
static int uECC_sign_with_k(const unsigned char private_key[uECC_BYTES],
    ...
    for (tries = 0; tries < MAX_TRIES; ++tries) {
        if (!g_rng_function((unsigned char *)tmp, sizeof(tmp))) {
            continue;
        }
        carry = 1;
        if (!vli_isZero(tmp)) {
...
int uECC_sign(const unsigned char private_key[uECC_BYTES],
    ...
    for (tries = 0; tries < MAX_TRIES; ++tries) {
        if(g_rng_function((unsigned char *)k, sizeof(k))) {
        #if (uECC_CURVE == uECC_secp160r1)
            k[uECC_WORDS] &= 0x01;
        #endif
...
static void HMAC_init(uECC_HashContext *hash_context, const unsigned char *K) {
    unsigned char *pad = hash_context->tmp + 2 * hash_context->result_size;
    unsigned i;
...
static void HMAC_finish(uECC_HashContext *hash_context, const unsigned char *K, unsigned char *result) {
    unsigned char *pad = hash_context->tmp + 2 * hash_context->result_size;
    unsigned i;
    for (i = 0; i < hash_context->result_size; ++i)
        pad[i] = K[i] ^ 0x5c;
...
/* V = HMAC_K(V) */
static void update_V(uECC_HashContext *hash_context, unsigned char *K, unsigned char *V) {
...
int uECC_sign_deterministic(const unsigned char private_key[uECC_BYTES],
                            ...
                            unsigned char signature[uECC_BYTES*2]) {
    unsigned char *K = hash_context->tmp;
    unsigned char *V = K + hash_context->result_size;
    uECC_word_t tries;
    unsigned i;
    for (i = 0; i < hash_context->result_size; ++i) {
        V[i] = 0x01;
